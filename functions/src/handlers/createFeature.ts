...
export const insertPolyActions = async (trx: Knex.Transaction, featureId: number, actions: PolyAction[]) => {
  for (const action of actions) {
    ...
    for (const treatment of action.treatments ?? []) {
      ...
      for (const herbicide of treatment.herbicides ?? []) {
  ...
  if (table === 'POLY') {
    const overlapResult = await trx.raw<{ overlap_count: number }[]>(
      `SELECT COUNT(*) as overlap_count
       FROM [dbo].[POLY] p
       WHERE LOWER(p.TypeDescription) = ? AND p.Project_ID = ?
...
export const createFeatureHandler = async ({ data }: CallableRequest) => {
  ...
  try {
    ...
    const featureType = data?.featureType?.toString() ?? '';
    const key = data?.key?.toString() ?? null;
    const token = data?.token?.toString() ?? null;
    const retreatment = (data?.retreatment?.toString().toUpperCase() ?? 'N') as 'Y' | 'N';
    const actions = (data?.actions ?? null) as PolyAction[] | PointLineAction[] | null;
    const geometryData = data?.geometry as object | null;
