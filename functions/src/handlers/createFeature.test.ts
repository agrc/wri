...
describe('createFeatureHandler', () => {
  ...
  it('throws invalid-argument when retreatment is set for an unsupported polygon type', async () => {
    await expect(
  ...
  it('allows retreatment for aquatic treatment areas', async () => {
    ...
    vi.mocked(getDb).mockResolvedValue({
      transaction: vi.fn().mockResolvedValue({ featureId: 123, statusDescription: 'Active' }),
    } as never);
    ...
    expect(result).toMatchObject({ featureId: 123, statusDescription: 'Active' });
  });

  it('defaults retreatment to false when omitted', async () => {
    vi.mocked(canEditProject).mockResolvedValue(true);
    vi.mocked(getDb).mockResolvedValue({
      transaction: vi.fn().mockResolvedValue({ featureId: 123, statusDescription: 'Active' }),
    } as never);
    ...
    const result = await createFeatureHandler({
      data: { ...validPolyData, retreatment: undefined },
    } as never);
...
describe('createFeatureTransaction', () => {
  ...
  it('skips AREAHERBICIDE inserts when no herbicide is provided', async () => {
    ...
    await createFeatureTransaction(
      ...
      'POLYGON((0 0, 1 0, 1 1, 0 0))',
      'N',
      validPolyActionsWithoutHerbicide,
      1000,
      null,
      mockEmptyIntersections,
    );
    ...
    const herbicideInserts = insertCalls.filter((c) => c.table === 'AREAHERBICIDE');
    expect(herbicideInserts).toHaveLength(0);
  });
